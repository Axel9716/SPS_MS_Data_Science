...
def exercise13(n):
    ...
    s = pd.Series(np.random.randint(0, n, size=n), index=date_index)
    s.cumsum().plot()
    plt.title('Cumulative Sum')
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('ex13_plot.png')
    # ------ Place code above here /\ /\ /\ ------ 
    return s
...
def exercise14(words):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df = pd.Series(words).map(lambda x: len(x)).to_frame(name='length')
    # ------ Place code above here /\ /\ /\ ------ 
    return df
