...
*Answer: A streak length of 1 means that he had 1 H, hit, followed by a M, miss. A streak length of 0 means he had 1 M preceeded by 0 H's.*
...
*Answer: The distribution of Kobe's streaks is heavily right skewed. Most, almost 40, of his streaks were 0. His longest streak of baskets was 4, only about 1 or 2 times.*

```{r}
#| label: kobe-streak-distribution

kobe_streak |> 
  ggplot(aes(x = length)) +
  geom_bar() +
  labs(title = "Kobe Shooting Streak Distribution")
```
...
*Answer: In my simulation, 25 flips came up heads.*

```{r}
#| label: sample-unfair-coin
...
```{r set-seed}
set.seed(606)                 # make sure to change the seed
```
...
```{r}
#| label: sim-kobe-shooting-percentage
...
*Answer: The distribution of `sim_streak` lengths is right skewed. Most of the streak lengths are 0 or 1, at counts of about 33 and 27 respectively. The longest streak is 4 at a count of about 3.*

```{r}
sim_streak |> 
  ggplot(aes(x = length)) +
  geom_bar() +
  labs(title = "Simulation Shooting Streak Distribution")
```
...
*Answer: I would expect the new simulation to be similar to the original one. The probability of missing is 55% and hitting one basket (length = 1) is 45%, hitting 2 in a row is 20%, 3 in a row is 9%, 4 in a row is 4%. Therefore, we can reasonably expect the distribution to be right skewed. However, it is not guaranteed. It is also to possible to have a bimodal distribution with peaks of 0 and 5, or even 133 0's or 1 133 count, though the latter examples are extremely unlikely.*
...
*Answer: Kobe's streak distribution is fairly similar to the the simulation streak distribution. This is evidence that the hot hand model does not fit Kobe's shooting patterns, because this comparison shows the Kobe's shooting pattern was similar to that of a simulation of an independent 45% shooter, his average. If he had a "hot hand", I would expect his streak distribution to be less right skewed.*
