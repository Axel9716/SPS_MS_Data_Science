...
*Answer: There are 123 variables and 1,458 observations.*
...
*Answer: For plotting 2 numerical variables, we can use a scatter plot.
The relationship between `pf_score` and `pf_expression_control` does
look linear. I would be comfortable using a linear model to predict the
`pf_score` based on `pf_expression_control`.*
...
*Answer: The relationship of these two variables looks strong, linear,
and positive. There are also vertical patterns, which indicate many
identical y values within most x values.*
...
*Answer: The lowest sum of squares I could get is 953. Another student
got 955, which is very close to my result.*
...
*Answer: `hf_score` = 5.154 + 0.350 \* `pf_expression_control`. The slope tells us that for every increase of 1 in `pf_expression_control` increases `hf_score` by 0.350. *
...
*Answer: At a `pf_expression_control` rating of 6.7, the regression line gives us a `pf_score` estimate of about 7.9. This would appear to be a slight overestimate. It is hard to tell because there are multiple y-values above and below our estimate, but there appears to be a stronger pull below our estimate.*
...
*Answer: There doesn't appear to be any pattern in the residuals plot. This satisifies one of the conditions for assessing the viability of a linear model.*
...
*Answer: The nearly normal residuals condition appears to be met.*
...
*Answer: Yes, the constant variability of residuals condition appears to be met.*
...
*Answer: There does appear to be a linear relationship between `pf_ss` and `pf_ss_women`.*

```{r}
hfi |> 
  ggplot(aes(x = pf_ss, y = pf_ss_women)) +
  geom_point() +
...
*Answer: My variables have a weaker relationship. My independent variable is not as good a predictor, because the R-squared for my model is 0.310, half of the R-squared for `pf_expression_control` ~ `pf_score`.*

```{r}
m3 <- lm(pf_ss_women ~ pf_ss, data = hfi)
...
*Answer: I was most surprised by the relationship between `pf_ss_women` and `pf_ss` because I expected them to have a stronger linear relationship. Based on the diagnostics below, a linear model appears to be appropriate for these variables.*
