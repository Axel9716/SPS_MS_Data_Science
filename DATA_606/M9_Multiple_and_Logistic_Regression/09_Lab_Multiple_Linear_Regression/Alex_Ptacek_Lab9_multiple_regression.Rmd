...
*Answer: The distribution of scores is between 2 and 5 and is centered
around 4. It is right skewed, meaning most students give a higher grade
between 4 and 5. I was expecting scores to be normally distributed as I
would imagine 3 to be the average.*
...
*Students in upper level classes tend to rate their professors as being
better looking than lower level classes.*

```{r}
evals |> 
  ggplot(aes(x = cls_level, y = bty_avg)) +
...
*Answer: Looks like the points in the initial plot were rounded, which
made the plot look too uniform and not truly representative of the
data.*
...
*Average beauty is a significant predictor of professor score because of
the very low p-value, but it does not seem to be practically significant
because R-squared is very low at .035.*
...
*Answer: The residual plot against fitted values show the residuals have
no pattern and the distribution of residuals is not normal.*
...
*Answer: Yes, bty_avg still looks like a significant predictor and its
parameter estimate has changed with the addition of gender.*
...
*Answer: score = y_intercept + color_hat \* bty_hat. Black and white tend to have higher scores than color pictures.*
...
*Answer: R uses tenure track and tenured as the predictor variables and uses teaching as the reference level.*

```{r}
m_bty_rank <- lm(score ~ rank, evals)
summary(m_bty_rank)
```
...
*Answer: I would expect class size to have the highest p-value.*
...
*Answer: Number of Professors has the highest p-value.*
...
*Answer: Professors that are not ethnically a minority have a higher score, given all other variables remain the same.*
...
*Answer: The coefficent estimates hardly changed, meaning that the dropped variable was not collinear with the other explanatory variables.*
...
*Answer: I could not increase the R-square by remove any variable from the previous model.*

```{r}
best_model <- lm(score ~ rank + gender + ethnicity + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(best_model)
```
...
*Answer: Yes, there could be observations not independent of one another, i.e. prof_id and course_id.*
...
*Answer: Professors had higher scores correlating with teaching rank, male, not minority, english school, higher percent of completed evals, more students in the class, upper level classes, one credit, higher beauty score, formal pic outfit, and black and white photo. Of particular interest is that male and one course credit have the lowest p-values.*
...
*Answer: No, I think only having 6 students rank the appearance of the professor is not a large enough sample size.*
