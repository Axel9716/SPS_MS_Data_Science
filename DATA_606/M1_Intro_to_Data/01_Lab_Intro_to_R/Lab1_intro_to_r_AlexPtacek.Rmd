...
```{r view-girls-answer, results='hide'}
arbuthnot$girls
```
...
##### Plotting and Interpreting Girls Baptisms from the Arbuthnot Dataset
The code below will create a line graph of girls baptized in London each year based on the Arbuthnot dataset.
```{r girls-text-answer, eval=FALSE, results='hide'}
ggplot(data=arbuthnot, aes(x=year, y = girls)) + geom_line()
```

|           We can see that the amount of girls being baptised in this dataset appears to trend gently upwards until 1640, followed by a sudden steep decrease that lasts until about 1659. This is followed by another sudden and steep, but positive, change in the number of girls baptised until about 1663. From this point, until the end of the dataset in 1710, there again appears to be an upwards trend overall. I would hypothesize from this that there was likely some significant event taking place between 1640 and 1659, such as a war or disease, or perhaps a religious purge that forbid children from being baptized, and that excluding this event, the amount of girls being baptized per year tends to increase over time.

```{r girls-graph-ansewer, echo=FALSE}
ggplot(data=arbuthnot, aes(x=year, y = girls)) + geom_line()
```
...
Similarly to the way we computed the total number of births, you can compute the ratio of the number of boys to the number of girls baptized in 1629 with
...
|         The plot below shows us that there were more boys baptized than girls for every year in this dataset, but never surpassing 54% of the total in a year. Most years, boys made up between 51% and 53% of the total, and the average proportion is likely around 52%. It is hard to identify a clear over-time trend, visually, but it looks slightly negative. We could possibly fit a cosine function here.
          
```{r boy_ratio_plot_answer, echo=FALSE}
...
The 'present' dataset includes the years 1940 - 2002. The dataset has 63 observations of 3 variables. The variable names are 'year', 'boys', and 'girls'.

```{r glimpse-present-data}
present$year
present %>%
  summarize(start_year = min(year), last_year = max(year))
glimpse(present)
```
...
The 'arbuthnot' dataset has 19 more observations than the 'present' dataset, which represents a 26% difference in the counts of observations. This can be considered large difference.

```{r observation-compare}
count(arbuthnot) - count(present)
(count(arbuthnot) - count(present)) / ((count(arbuthnot) + count(present)) / 2)
```

***

The updated 'arbuthnot' dataset has 4 more variables than the 'present' dataset. This represents a 80% difference in counts of variables and can be considered a large difference.

```{r variables-compare}
ncol(arbuthnot) - ncol(present)
(ncol(arbuthnot) - ncol(present)) / ((ncol(arbuthnot) + ncol(present)) / 2) 
```

***

On average, the total count of boys and girls per year in 'present' are 321 times higher than that of 'arbuthnot'. This is a very large change.
...
```{r comparing-means}
(mean(present$total) - mean(arbuthnot$total)) / mean(arbuthnot$total)
```

**In conclusion, multiple counts of 'arbuthnot' and 'present' vary in great magnitude.**
...
Fascinatingly, several of the observations from 'arbuthnot' are similar to 'present'. Specifically, the fact that every year the count of boys is higher than that of girls is true in both datasets. Additionally, 'present' more clearly looks like a cosine function.

```{r plot-present-ratio}
present <- present %>%
  mutate(boy_ratio = boys / total)
...
We saw the most total number of births in the U.S. (4,268,326) in the year 1961.

```{r arrange-present-total, echo = FALSE}
present %>%
  arrange(desc(total))
```
